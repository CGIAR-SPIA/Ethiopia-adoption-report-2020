...
recode SEX (2=0)
edit SEX
label define s1q02 1 "1. Male" 0 "2. Female", replace

gen aefs=.

recode aefs (.=0.33) if SEX==0 & (AGE<1)  /*under 1 year*/
recode aefs (.=0.33) if SEX==1 & (AGE<1)  /*under 1 year*/


recode aefs (.=0.46) if SEX==0 & (AGE>=1 & AGE <=1.99)  /*for 1 to 1.99 years*/
recode aefs (.=0.46) if SEX==1 & (AGE>=1 & AGE <=1.99)  /*for 1 to 1.99 years*/


recode aefs (.=0.54) if SEX==0 & (AGE>=2 & AGE <=2.99)  /*for 2 to 2.99 years*/
recode aefs (.=0.54) if SEX==1 & (AGE>=2 & AGE <=2.99)  /*for 2 to 2.99 years*/


recode aefs (.=0.62) if SEX==0 & (AGE>=3 & AGE <=4.99)  /*for 3 to 4.99 years*/
recode aefs (.=0.62) if SEX==1 & (AGE>=3 & AGE <=4.99)  /*for 3 to 4.99 years*/


recode aefs (.=0.70) if SEX==0 & (AGE>=5 & AGE <=6.99)  /*for 5 to 6.99 years*/
recode aefs (.=0.74) if SEX==1 & (AGE>=5 & AGE <=6.99)  /*for 5 to 6.99 years*/


recode aefs (.=0.72) if SEX==0 & (AGE>=7 & AGE <=9.99)  /*for 7 to 9.99 years*/
recode aefs (.=0.84) if SEX==1 & (AGE>=7 & AGE <=9.99)  /*for 7 to 9.99 years*/

recode aefs (.=0.78) if SEX==0 & (AGE>=10 & AGE <=11.99)  /*for 10 to 11.99 years*/
recode aefs (.=0.88) if SEX==1 & (AGE>=10 & AGE <=11.99)  /*for 10 to 11.99 years*/


recode aefs (.=0.84) if SEX==0 & (AGE>=12 & AGE <=13.99)  /*for 12 to 13.99 years*/
recode aefs (.=0.96) if SEX==1 & (AGE>=12 & AGE <=13.99)  /*for 12 to 13.99 years*/


recode aefs (.=0.86) if SEX==0 & (AGE>=14 & AGE <=15.99)  /*for 14 to 15.99 years*/
recode aefs (.=1.06) if SEX==1 & (AGE>=14 & AGE <=15.99)  /*for 14 to 15.99 years*/


recode aefs (.=0.86) if SEX==0 & (AGE>=16 & AGE <=17.99)  /*for 16 to 17.99 years*/
recode aefs (.=1.14) if SEX==1 & (AGE>=16 & AGE <=17.99)  /*for 16 to 17.99 years*/



recode aefs (.=0.80) if SEX==0 & (AGE>=18 & AGE <=29.99)  /*for 18 to 29.99 years*/
recode aefs (.=1.04) if SEX==1 & (AGE>=18 & AGE <=29.99)  /*for 18 to 29.99 years*/


recode aefs (.=0.82) if SEX==0 & (AGE>=30 & AGE <=59.99)  /*for 30 to 59.99 years*/
recode aefs (.=0.1) if SEX==1 & (AGE>=30 & AGE <=59.99)  /*for 30 to 59.99 years*/


recode aefs (.=0.74) if SEX==0 & (AGE>=60)  /*60+*/
recode aefs (.=0.84) if SEX==1 & (AGE>=60 )  /*60+*/
*======================================================================================

replace aefs=. if AGE==.
replace aefs=. if SEX==.
egen aeu_tot =sum ( aefs ), by( household_id)
replace aeu_tot=. if aefs==.
collapse (mean) aeu_tot , by (household_id)
lab var aeu_tot  "Adult equivalent unit"


**** age chatagpories

clonevar s1q03a2=s1q03a

replace s1q03a2=1 if s1q03a2<5 
replace s1q03a2=2 if s1q03a2<=17 & s1q03a2 >=5
replace s1q03a2=3 if s1q03a2<=29 & s1q03a2 >=15 // according to FDRI youth policy (https://www.usaid.gov/sites/default/files/documents/1860/Fact_Sheet_Developing_Ethiopias_Youth_Jul_2017.pdf)
replace s1q03a2=4 if s1q03a2<=40 & s1q03a2 >=30
replace s1q03a2=5 if s1q03a2<=55 & s1q03a2 >=41
replace s1q03a2=6 if s1q03a2>=56 & s1q03a2!=.

label define s1q03a2  1 "<5" 2 "5-17"  3"15-29" 4 "30-40" 5 "41-55" 6 ">55", modify
label val s1q03a2 s1q03a2
ta s1q03a2
ta s1q03a2, gen(s1q03a2)
